...
const voice = (id, language, gender, source='system') => ({id,name:id,facets:{language:[language],gender:[gender],source:[source],style:['温柔']},official:{voice_id:id}});
const azure = [voice('zh-CN-XiaochenNeural','zh-CN','Female'),voice('中文男声','zh-CN','Male'),voice('English','en-US','Female')];
...
(async()=>{
 ...
 assert.equal(d.querySelectorAll('select[data-key]').length,6);
 ...
 $('filter-gender').value='Female';$('filter-gender').dispatchEvent(new w.Event('change'));
