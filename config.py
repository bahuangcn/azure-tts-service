...
# Load every setting before paths are evaluated; never override process environment.
...
# JSON mapping: client name -> random secret. No anonymous access if absent.
API_KEYS = __import__("json").loads(os.environ.get("TTS_API_KEYS", "{}"))
if not isinstance(API_KEYS, dict) or any(not isinstance(v, str) or len(v) < 32 for v in API_KEYS.values()):
    raise RuntimeError("TTS_API_KEYS must map client names to secrets of at least 32 characters")
CORS_ORIGINS = os.environ.get("TTS_CORS_ORIGINS", "https://tools.exnihilo.site").split(",")
MAX_PENDING = int(os.environ.get("TTS_MAX_PENDING", "100"))
RATE_LIMIT = int(os.environ.get("TTS_RATE_LIMIT", "20"))
